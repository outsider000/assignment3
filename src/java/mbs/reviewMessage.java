...
public class reviewMessage extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try{
            ...
            String q1 = new String("SELECT * FROM ss_message WHERE username = '"+
    ...
    try{
       if( rs.next())
       {
            out.println("</tr><td>Welcome " + rs.getString("username")+ "</td><tr>");
            String q2 = new String("SELECT * FROM ss_message WHERE password = '"+
            request.getParameter("password")+"'AND username = '"+
            request.getParameter("username")+"'");
            rs =  st.executeQuery(q2);
                   if( rs.next()){
                    out.println("</tr><td>Login success</td><tr>");  
                   ...
                   out.println("</tr><td>Incorrect password</td><tr>");  
                                       RequestDispatcher rd = getServletContext().getRequestDispatcher("/welcome.jsp");
                    rd.forward(request, response);
                   }
       }
       ...
       {
                       ub.setStatus("LOGIN FAILURE RETRY");
            out.println("</tr><td>No User Found</td><tr>");   
                                RequestDispatcher rd = getServletContext().getRequestDispatcher("/welcome.jsp");
                    rd.forward(request, response);
       }
